...
const struct decoded Decode[] = {
    ...
    OP(IN_ORA, AM_ABSY, FOO, PG(4)), // 1D - ORA abs,X
    OP(IN_ASL, AM_ABSY, FOO, CY(7)), // 1E - ASL abs,X
    UP(IN_SLO, AM_ABSY, FOO, CY(7)), // 1F - *SLO (ASO) abs,X
    OP(IN_JSR, AM_JSR, FOO, CY(6)),  // 20 - JSR
    OP(IN_AND, AM_INDX, FOO, CY(6)), // 21 - AND (zp,X)
    JAM,                        // 22 - *JAM (KIL, HLT, CIM, CRP)
    UP(IN_RLA, AM_INDX, FOO, CY(8)), // 23 - *RLA (RLN) (zp,X)
    OP(IN_BIT, AM_ZP, FOO, CY(3)),   // 24 - BIT zp
    OP(IN_AND, AM_ZP, FOO, CY(3)),   // 25 - AND zp
    OP(IN_ROL, AM_ZP, FOO, CY(5)),   // 26 - ROL zp
    UP(IN_RLA, AM_ZP, FOO, CY(5)),   // 27 - *RLA (RLN) zp
    OP(IN_PLP, AM_PLL, FOO, CY(4)),  // 28 - PLP
    OP(IN_AND, AM_IMM, FOO, CY(2)),  // 29 - AND imm
    OP(IN_ROL, AM_IMP, FOO, CY(2)),  // 2A - ROL imp
    UP(IN_ANC, AM_IMM, FOO, CY(2)),  // 2B - *ANC (ANC2) imm
    OP(IN_BIT, AM_ABS, FOO, CY(4)),  // 2C - BIT abs
    OP(IN_AND, AM_ABS, FOO, CY(4)),  // 2D - AND abs
    OP(IN_ROL, AM_ABS, FOO, CY(6)),  // 2E - ROL abs
    UP(IN_RLA, AM_ABS, FOO, CY(6)),  // 2F - *RLA (RLN) abs
    OP(IN_BMI, AM_BCH, FOO, BR(2)),  // 30 - BMI
    OP(IN_AND, AM_INDY, FOO, PG(5)), // 31 - AND (zp),Y
    JAM,                        // 32 - *JAM (KIL, HLT, CIM, CRP)
    UP(IN_RLA, AM_INDY, FOO, CY(8)), // 33 - *RLA (RLN) (zp),Y
    UP(IN_NOP, AM_ZPX, FOO, CY(4)),  // 34 - *NOP (DOP, SKB, IGN) zp,X
    OP(IN_AND, AM_ZPX, FOO, CY(4)),  // 35 - AND zp,X
    OP(IN_ROL, AM_ZPX, FOO, CY(6)),  // 36 - ROL zp,X
    UP(IN_RLA, AM_ZPX, FOO, CY(6)),  // 37 - *RLA (RLN) zp,X
    OP(IN_SEC, AM_IMP, FOO, CY(2)),  // 38 - SEC
    OP(IN_AND, AM_ABSY, FOO, PG(4)), // 39 - AND abs,Y
    ...
    OP(IN_ADC, AM_ABSY, FOO, PG(4)), // 7D - ADC abs,X
    OP(IN_ROR, AM_ABSY, FOO, CY(7)), // 7E - ROR abs,X
    ...
    OP(IN_TXA, AM_IMP, FOO, CY(2)),  // 8A - TXA
    ...
    OP(IN_BCC, AM_BCH, FOO, BR(2)),  // 90 - BCC
    ...
    OP(IN_TYA, AM_IMP, FOO, CY(2)),  // 98 - TYA
    ...
    OP(IN_CMP, AM_ABSY, FOO, PG(4)), // DD - CMP abs,X
    OP(IN_DEC, AM_ABSY, FOO, CY(7)), // DE - DEC abs,X
    UP(IN_DCP, AM_ABSY, FOO, CY(7)), // DF - *DCP (DCM) abs,X
    OP(IN_CPX, AM_IMM, FOO, CY(2)),  // E0 - CPX imm
    OP(IN_SBC, AM_INDX, FOO, CY(6)), // E1 - SBC (zp,X)
    UP(IN_NOP, AM_IMM, FOO, CY(2)),  // E2 - *NOP (DOP, SKB) imm (?)
    UP(IN_ISC, AM_INDX, FOO, CY(8)), // E3 - *ISC (ISB, INS) (zp,X)
    OP(IN_CPX, AM_ZP, FOO, CY(3)),   // E4 - CPX zp
    OP(IN_SBC, AM_ZP, FOO, CY(3)),   // E5 - SBC zp
    OP(IN_INC, AM_ZP, FOO, CY(5)),   // E6 - INC zp
    UP(IN_ISC, AM_ZP, FOO, CY(5)),   // E7 - *ISC (ISB, INS) zp
    OP(IN_INX, AM_IMP, FOO, CY(2)),  // E8 - INX
    OP(IN_SBC, AM_IMM, FOO, CY(2)),  // E9 - SBC imm
    OP(IN_NOP, AM_IMP, FOO, CY(2)),  // EA - NOP
    UP(IN_SBC, AM_IMM, FOO, CY(2)),  // EB - *SBC (USBC, USB) imm
    OP(IN_CPX, AM_ABS, FOO, CY(4)),  // EC - CPX abs
    OP(IN_SBC, AM_ABS, FOO, CY(4)),  // ED - SBC abs
    OP(IN_INC, AM_ABS, FOO, CY(6)),  // EE - INC abs
    UP(IN_ISC, AM_ABS, FOO, CY(6)),  // EF - *ISC (ISB, INS) abs
    OP(IN_BEQ, AM_BCH, FOO, BR(2)),  // F0 - BEQ
    OP(IN_SBC, AM_INDY, FOO, PG(5)), // F1 - SBC (zp),Y
    ...
    OP(IN_SBC, AM_ZPX, FOO, CY(4)),  // F5 - SBC zp,X
    OP(IN_INC, AM_ZPX, FOO, CY(6)),  // F6 - INC zp,X
    UP(IN_ISC, AM_ZPX, FOO, CY(6)),  // F7 - *ISC (ISB, INS) zp,X
    OP(IN_SED, AM_IMP, FOO, CY(2)),  // F8 - SED
    OP(IN_SBC, AM_ABSY, FOO, PG(4)), // F9 - SBC abs,Y
    ...
    OP(IN_SBC, AM_ABSY, FOO, PG(4)), // FD - SBC abs,X
