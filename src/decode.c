...
const struct decoded Decode[] = {
    ...
    UP(IN_NOP, AM_ZP),      // 04 - *NOP (DOP, SKB, IGN) zp
    ...
    UP(IN_ANC, AM_IMM),     // 0B - *ANC (ANA, ANB) imm
    UP(IN_NOP, AM_ABS),     // 0C - *NOP (TOP, SKW, IGN) abs
    ...
    UP(IN_NOP, AM_ZPX),     // 14 - *NOP (DOP, SKB, IGN) zp,X
    ...
    UP(IN_NOP, AM_IMP),     // 1A - *NOP (NPO, UNP)
    UP(IN_SLO, AM_ABSY),    // 1B - *SLO (ASO) abs,Y
    UP(IN_NOP, AM_ABSX),    // 1C - *NOP (TOP, SKW, IGN) abs,X
    ...
    UP(IN_NOP, AM_ZPX),     // 54 - *NOP (DOP, SKB, IGN) zp,X
    ...
    UP(IN_NOP, AM_IMP),     // 5A - *NOP (NPO, UNP)
    UP(IN_SRE, AM_ABSY),    // 5B - *SRE (LSE) abs,Y
    UP(IN_NOP, AM_ABSX),    // 5C - *NOP (TOP, SKW, IGN) abs,X
    ...
    UP(IN_RRA, AM_INDX),    // 63 - *RRA (RLD) (zp,X)
    UP(IN_NOP, AM_ZP),      // 64 - *NOP (DOP, SKB, IGN) zp
    OP(IN_ADC, AM_ZP),      // 65 - ADC zp
    OP(IN_ROR, AM_ZP),      // 66 - ROR zp
    UP(IN_RRA, AM_ZP),      // 67 - *RRA (RLD) zp
    ...
    UP(IN_RRA, AM_ABS),     // 6F - *RRA (RLD) abs
    OP(IN_BVS, AM_BCH),     // 70 - BVS
    OP(IN_ADC, AM_INDY),    // 71 - ADC (zp),Y
    JAM,                    // 72 - *JAM (KIL, HLT, CIM, CRP)
    UP(IN_RRA, AM_INDY),    // 73 - *RRA (RLD) (zp),Y
    UP(IN_NOP, AM_ZPX),     // 74 - *NOP (DOP, SKB, IGN) zp,X
    OP(IN_ADC, AM_ZPX),     // 75 - ADC zp,X
    OP(IN_ROR, AM_ZPX),     // 76 - ROR zp,X
    UP(IN_RRA, AM_ZPX),     // 77 - *RRA (RLD) zp,X
    OP(IN_SEI, AM_IMP),     // 78 - SEI
    OP(IN_ADC, AM_ABSY),    // 79 - ADC abs,Y
    UP(IN_NOP, AM_IMP),     // 7A - *NOP (NPO, UNP)
    UP(IN_RRA, AM_ABSY),    // 7B - *RRA (RLD) abs,Y
    UP(IN_NOP, AM_ABSX),    // 7C - *NOP (TOP, SKW, IGN) abs,X
    OP(IN_ADC, AM_ABSX),    // 7D - ADC abs,X
    OP(IN_ROR, AM_ABSX),    // 7E - ROR abs,X
    UP(IN_RRA, AM_ABSX),    // 7F - *RRA (RLD) abs,X
    UP(IN_NOP, AM_IMM),     // 80 - *NOP (DOP, SKB) imm
    OP(IN_STA, AM_INDX),    // 81 - STA (zp,X)
    UP(IN_NOP, AM_IMM),     // 82 - *NOP (DOP, SKB) imm (?)
    ...
    UP(IN_NOP, AM_IMM),     // 89 - *NOP (DOP, SKB) imm
    OP(IN_TXA, AM_IMP),     // 8A - TXA
    UP(IN_ANE, AM_IMM),     // 8B - *ANE (XAA, AXM) imm (!!)
    ...
    UP(IN_SHA, AM_INDY),    // 93 - *SHA (AHX, AXA, TEA) (zp),Y (!)
    ...
    UP(IN_TAS, AM_ABSY),    // 9B - *TAS (XAS, SHS) abs,Y (!)
    UP(IN_SHY, AM_ABSX),    // 9E - *SHY (A11, SYA, SAY, TEY) abs,X (!)
    OP(IN_STA, AM_ABSX),    // 9D - STA abs,X
    UP(IN_SHX, AM_ABSY),    // 9E - *SHX (A11, SXA, XAS, TEX) abs,Y (!)
    UP(IN_SHA, AM_ABSY),    // 9F - *SHA (AHX, AXA, TEA) abs,Y (!)
    ...
    UP(IN_LXA, AM_IMM),     // AB - *LXA (LAX, ATX, OAL, ANX) imm (!!)
    ...
    UP(IN_NOP, AM_IMM),     // C2 - *NOP (DOP, SKB) imm (?)
    ...
    UP(IN_SBX, AM_IMM),     // CB - *SBX (AXS, SAX, XMA) imm
    ...
    UP(IN_NOP, AM_ZPX),     // D4 - *NOP (DOP, SKB, IGN) zp,X
    ...
    UP(IN_NOP, AM_IMP),     // DA - *NOP (NPO, UNP)
    UP(IN_DCP, AM_ABSY),    // DB - *DCP (DCM) abs,Y
    UP(IN_NOP, AM_ABSX),    // DC - *NOP (TOP, SKW, IGN) abs,X
    ...
    UP(IN_NOP, AM_IMM),     // E2 - *NOP (DOP, SKB) imm (?)
    ...
    UP(IN_SBC, AM_IMM),     // EB - *SBC (USBC, USB) imm
    ...
    UP(IN_NOP, AM_ZPX),     // F4 - *NOP (DOP, SKB, IGN) zp,X
    ...
    UP(IN_NOP, AM_IMP),     // FA - *NOP (NPO, UNP)
    UP(IN_ISC, AM_ABSY),    // FB - *ISC (ISB, INS) abs,Y
    UP(IN_NOP, AM_ABSX),    // FC - *NOP (TOP, SKW, IGN) abs,X
