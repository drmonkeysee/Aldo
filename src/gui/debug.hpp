...
public:
    ...
    void addBreakpoint(haltexpr expr) noexcept
    {
        debug_bp_add(debugp(), expr);
    }
    void toggleBreakpointEnabled(et::diff at) noexcept
    {
        const auto bp = getBreakpoint(at);
        debug_bp_enabled(debugp(), at, bp && !bp->enabled);
    }
    void removeBreakpoint(et::diff at) noexcept
    {
        debug_bp_remove(debugp(), at);
    }
    void clearBreakpoints() noexcept { debug_bp_clear(debugp()); }
    ...
    void exportBreakpoints(const std::filesystem::path& filepath);
