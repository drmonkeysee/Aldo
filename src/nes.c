...
struct aldo_nes001 {
    ...
    struct aldo_rp2a03 chip;    // RP2A03 Microprocessor
...
static bool create_mbus(struct aldo_nes001 *self)
{
     ...
     *   $8000 - $FFFF: 32KB Cart
     */
    self->chip.cpu.mbus = aldo_bus_new(ALDO_BITWIDTH_64KB, 4, ALDO_MEMBLOCK_8KB,
                                       ALDO_MEMBLOCK_16KB, ALDO_MEMBLOCK_32KB);
    if (!self->chip.cpu.mbus) return false;

    auto r = aldo_bus_set(self->chip.cpu.mbus, 0, (struct aldo_busdevice){
        ram_read,
        ram_write,
        ram_copy,
        self->ram,
...
{
    self->cart = c;
    auto r = aldo_cart_mbus_connect(self->cart, self->chip.cpu.mbus);
    (void)r, assert(r);
...
static void disconnect_cart(struct aldo_nes001 *self)
{
    ...
    aldo_cart_mbus_disconnect(self->cart, self->chip.cpu.mbus);
    self->cart = nullptr;
}
...
static bool setup(struct aldo_nes001 *self)
{
    ...
    aldo_ppu_connect(&self->ppu, self->chip.cpu.mbus);
    aldo_debug_cpu_connect(self->dbg, &self->chip.cpu);
    return true;
}
...
static void teardown(struct aldo_nes001 *self)
{
    ...
    aldo_bus_free(self->chip.cpu.mbus);
}
...
static void set_ppu_pins(struct aldo_nes001 *self)
{
    ...
    self->ppu.signal.rw |= self->chip.cpu.signal.rw;
}
...
{
    self->chip.cpu.signal.rdy = self->probe.rdy;
    // interrupt lines are active low
    self->chip.cpu.signal.irq = !self->probe.irq;
    self->chip.cpu.signal.nmi = !self->probe.nmi && self->ppu.signal.intr;
    self->chip.cpu.signal.rst = !self->probe.rst;
}
...
{
    aldo_chip_snapshot(&self->chip, snp);
    aldo_ppu_bus_snapshot(&self->ppu, snp);
    aldo_bus_copy(self->chip.cpu.mbus, ALDO_CPU_VECTOR_NMI,
...
static void snapshot_sys(struct aldo_nes001 *self)
{
    ...
    prg->curr->length = aldo_bus_copy(self->chip.cpu.mbus,
                              ...
                              const struct aldo_clock *clock, int adjustment)
{
    if (!self->tracelog || self->tracefailed
        || !self->chip.cpu.signal.sync) return;
...
{
    auto cycles = aldo_chip_cycle(&self->chip);
    set_cpu_pins(self);
    clock->subcycle = 0;
    ...
    case ALDO_EXC_CYCLE:
        ...
        break;
    case ALDO_EXC_STEP:
        aldo_nes_halt(self, self->chip.cpu.signal.sync);
        break;
    case ALDO_EXC_RUN:
    default:
        break;
