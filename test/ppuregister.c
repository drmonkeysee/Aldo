...
static void ppumask_probe_high(void *ctx)
{
    auto ppu = ppt_get_ppu(ctx);
    struct aldo_snapshot snp;
    aldo_ppu_set_probe(ppu, ALDO_PRB_PPU_GRAY, ALDO_TRIV_TRUE);
...
static void ppumask_probe_low(void *ctx)
{
    ...
    ppu->mask.g = true;
    struct aldo_snapshot snp;
    aldo_ppu_set_probe(ppu, ALDO_PRB_PPU_GRAY, ALDO_TRIV_FALSE);
...
static void ppudata_write_during_sprite_rendering(void *ctx)
{
    ...
    ct_assertequal(0x22u, ppu->pxpl.spu[0].fgs[0]);
