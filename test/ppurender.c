...
static void sprite_fetch_prerender_line(void *ctx)
{
    auto ppu = ppt_get_ppu(ctx);
    ppu->line = 261;
    ppu->dot = 257;
    ...
    ppu->oamaddr = 0xab;
    for (size_t i = 0; i < aldo_arrsz(ppu->spr.soam); i += 4) {
        // put one sprite randomly on line 5 which will be "in-range" on 261
        ppu->spr.soam[i] = i == 8 ? 5 : 239;        // y-coord
        ppu->spr.soam[i + 1] = (uint8_t)(i + 4);    // arbitrary tile id
        ppu->spr.soam[i + 2] = 0x2;                 // fv:0, hv:0, pri:fg, pal:2
        ppu->spr.soam[i + 3] = (uint8_t)(i + 23);   // arbitrary x-coord
    }
...
static void sprite_evaluation_partial_scanline(void *ctx)
{
    ...
    ct_assertequal(0u, spr->oamd);
    ct_assertfalse(spr->sprite0);   // no sprite-0 as oam[0] was not in range
    ct_assertequal(ALDO_PPU_SPR_DONE, (int)spr->s);
}
...
static void sprite_evaluation_oamaddr_offset(void *ctx)
{
    ...
    ct_assertequal(0u, ppu->oamaddr);
    ct_assertequal(0u, spr->oamd);
    ct_assertfalse(spr->sprite0);   // OAMADDR never touched sprite-0
    ct_assertequal(ALDO_PPU_SPR_DONE, (int)spr->s);
}
...
static void sprite_evaluation_oamaddr_misaligned(void *ctx)
{
    ...
    ct_assertequal(3u, ppu->oamaddr);
    ct_assertequal(0u, spr->oamd);
    ct_asserttrue(spr->sprite0);        // OAM[0][3] misinterpreted as y-coord
    ct_assertequal(ALDO_PPU_SPR_DONE, (int)spr->s);
}
...
static void sprite_evaluation_soamaddr_offset(void *ctx)
{
    ...
    // OAM[0] is in range though it likely isn't in SOAM[0] due to soaddr offset;
    // this will likely cause a glitchy sprite-0 hit check.
    ct_asserttrue(spr->sprite0);
    ct_assertequal(ALDO_PPU_SPR_DONE, (int)spr->s);
...
static void pixel_fg_sprite_opaque_bg(void *ctx)
{
    auto ppu = ppt_get_ppu(ctx);
    ppu->dot = 2;
    auto tlu = &ppu->pxpl.tlu;
...
static void pixel_bg_sprite_transparent_bg(void *ctx)
{
    auto ppu = ppt_get_ppu(ctx);
    ppu->dot = 2;
    auto tlu = &ppu->pxpl.tlu;
...
static void pixel_bg_sprite_opaque_bg(void *ctx)
{
    auto ppu = ppt_get_ppu(ctx);
    ppu->dot = 2;
    auto tlu = &ppu->pxpl.tlu;
...
static void pixel_fg_x_decrement(void *ctx)
{
    auto ppu = ppt_get_ppu(ctx);
    ppu->dot = 2;
    auto tlu = &ppu->pxpl.tlu;
    ...
    ct_assertequal(3, ppu->dot);
    ct_assertequal(0x8u, ppu->pxpl.mux);   // 0 10 00
    ct_assertequal(0u, ppu->pxpl.pal);
    ct_assertequal(0u, ppu->pxpl.px);
    ct_assertequal(0x6fu, spu->fgs[0]);
    ct_assertequal(0xcfu, spu->fgs[1]);
    ct_assertequal(1u, spu->x);
    ct_assertfalse(ppu->status.s);
    ct_assertfalse(ppu->signal.vout);
...
static void pixel_disabled_fg(void *ctx)
{
    auto ppu = ppt_get_ppu(ctx);
    ppu->dot = 66;
    auto spu = ppu->pxpl.spu;
...
static void left_mask_fg(void *ctx)
{
    auto ppu = ppt_get_ppu(ctx);
    ppu->line = 10;
    ppu->dot = 2;
...
struct ct_testsuite ppu_render_tests()
{
    static constexpr struct ct_testcase tests[] = {
        ...
        ct_maketest(sprite_fetch_prerender_line),
        ...
        ct_maketest(pixel_fg_sprite_opaque_bg),
        ct_maketest(pixel_bg_sprite_transparent_bg),
        ct_maketest(pixel_bg_sprite_opaque_bg),
